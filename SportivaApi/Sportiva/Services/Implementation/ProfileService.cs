    ...
    IHttpContextAccessor accessor) : IProfileService
{
        ...
        string profileUserId, string? currentUserId = null, CancellationToken ct = default)
    {
        try
        {
            ...
            var totalBookings = await _context.Bookings.CountAsync(b => b.UserId == profileUserId && !b.IsDeleted, ct);
            var completedBookings = await _context.Bookings.CountAsync(b => b.UserId == profileUserId && !b.IsDeleted && b.Status == BookingStatus.Completed, ct);
            var cancelledBookings = await _context.Bookings.CountAsync(b => b.UserId == profileUserId && !b.IsDeleted && b.Status == BookingStatus.Cancelled, ct);
            var totalTournaments = await _context.TournamentParticipants.CountAsync(tp => tp.UserId == profileUserId, ct);
            var totalMatches = await _context.MatchJoinRequests.CountAsync(mjr => mjr.PlayerId == profileUserId && mjr.Status == JoinRequestStatus.Accepted && mjr.FriendlyMatch.Date < today, ct);

            var attendanceRate = totalBookings > 0 ? (completedBookings * 100) / totalBookings : 98;
            var cancellationRate = totalBookings > 0 ? (cancelledBookings * 100) / totalBookings : 2;
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.UtcNow);
        var totalBookings = await _context.Bookings.CountAsync(b => b.UserId == userId && !b.IsDeleted, ct);
        var completedBookings = await _context.Bookings.CountAsync(b => b.UserId == userId && !b.IsDeleted && b.Status == BookingStatus.Completed, ct);
        var cancelledBookings = await _context.Bookings.CountAsync(b => b.UserId == userId && !b.IsDeleted && b.Status == BookingStatus.Cancelled, ct);
        var totalTournaments = await _context.TournamentParticipants.CountAsync(tp => tp.UserId == userId, ct);
        var totalMatches = await _context.MatchJoinRequests.CountAsync(mjr => mjr.PlayerId == userId && mjr.Status == JoinRequestStatus.Accepted && mjr.FriendlyMatch.Date < today, ct);
        ...
        string? currentUserId, RequestFilters filters, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var adminRole = await _context.Roles.FirstOrDefaultAsync(r => r.NormalizedName == "ADMIN", ct);
                if (adminRole != null)
                {
                    isAdmin = await _context.UserRoles.AnyAsync(ur => ur.UserId == currentUserId && ur.RoleId == adminRole.Id, ct);
                }
            }
            ...
            var mappedQuery = query.Select(u => new UserCardSummary(
                u.Id,
                u.FullName,
                ...
                u.Followers.Any(f => f.FollowerId == currentUserId), // IsFollowing
                u.Id == currentUserId,                               // IsMe
                u.Followers
                    .Where(f => f.FollowerId == currentUserId)
                    .Select(f => (DateTime?)f.FollowedAt)
                    .FirstOrDefault(),                               // FollowedAt
                u.IsDisabled                                         // IsDisabled
            ))
            .AsNoTracking();

            return await mappedQuery.ToPaginatedListAsync(filters, ct);
        }
        catch (Exception ex)
        {
    ...
    public async Task<Result> ToggleUserSuspensionAsync(
        string adminUserId, string targetUserId, CancellationToken ct = default)
    {
        try
        {
            ...
            if (target.Id == adminUserId)
                return Result.Failure(new Error("Admin.CannotSuspendSelf", "You cannot suspend your own administrative account.", StatusCodes.Status400BadRequest));

            var adminRole = await _context.Roles.FirstOrDefaultAsync(r => r.NormalizedName == "ADMIN", ct);
            if (adminRole != null)
            {
                var isTargetAdmin = await _context.UserRoles.AnyAsync(ur => ur.UserId == targetUserId && ur.RoleId == adminRole.Id, ct);
                if (isTargetAdmin)
                {
                    return Result.Failure(new Error("Admin.CannotSuspendAdmin", "Administrators cannot suspend other administrators.", StatusCodes.Status400BadRequest));
                }
            }
            ...
            _logger.LogInformation("Admin {AdminId} toggled suspension status for user {TargetId} to {Status}", adminUserId, targetUserId, target.IsDisabled);
            return Result.Success();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error occurred while toggling suspension status for user {TargetId}", targetUserId);
            return Result.Failure(new Error("Admin.SuspensionError", "An error occurred while modifying user suspension status.", StatusCodes.Status500InternalServerError));
        }
    }
