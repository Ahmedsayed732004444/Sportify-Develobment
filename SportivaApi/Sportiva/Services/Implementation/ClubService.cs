    ...
    IHttpContextAccessor accessor) : IClubService
{
        ...
        string ownerId, CreateClubRequest request, CancellationToken ct = default)
    {
        try
        {
            ...
            var currentClubsCount = await _context.Clubs
                .CountAsync(c => c.OwnerId == ownerId && !c.IsDeleted, ct);

            if (currentClubsCount > 0)
            {
                var existingClubs = await _context.Clubs
                ...
                var activeSubscription = await _context.ClubSubscriptions
                    .Include(s => s.Plan)
                    .Where(s => existingClubs.Contains(s.ClubId) && !s.IsDeleted &&
                                s.StartDate <= now && s.EndDate >= now)
                    .OrderByDescending(s => s.Plan.Price)
                    .FirstOrDefaultAsync(ct);
                ...
                if (activeSubscription != null)
                {
                    ...
                    if (planName.Contains("premium")) maxClubs = 2;
                    else if (planName.Contains("elite") || planName.Contains("unlimited")) maxClubs = 5;
                }

                if (currentClubsCount >= maxClubs)
                {
                    return Result.Failure<ClubResponse>(
                        new Error("Clubs.LimitReached", $"You have reached the maximum number of clubs ({maxClubs}) allowed by your current subscription plan.", StatusCodes.Status400BadRequest));
                }
            }
